...
def application_list(request):
    ...
    # Pass the voters data to the template for rendering
    return render(request, 'application_list.html', {'voters': voters})
...
def adminprofile(request):
    ...
    return render(request, 'admin/adminprofile.html',
                  {'username': username, 'firstname': firstname, 'lastname': lastname, 'middlename': middlename,
                   'gender': gender, 'dob': dateofbirth, 'address': address, 'pincode': pincode, 'mob': mobile_no,
                   'email': email, 'image': adminimage})
...
def add_voter(request):
    if (request.method == 'POST'):
        ...
        if (Voters.objects.filter(voterid_no=voterid_no).exists()):
            ...
            return render(request, 'admin/addvoter.html',
                          {'username': adminhome.username, 'image': adminhome.adminimage})
        else:
...
def add_candidate(request):
    if request.method == 'POST':
        ...
        if (Candidates.objects.filter(candidate_id=candidate_id).exists()):
            ...
            return render(request, 'admin/add candidate.html',
                          {'username': adminhome.username, 'image': adminhome.adminimage})
        else:
            if constituency == "Parliamentary":
...
def generateelection(request):
    return render(request, 'admin/generateelection.html',
                  {'username': adminhome.username, 'image': adminhome.adminimage})
    ...
    if request.method == 'POST':
        ...
        if (Election.objects.filter(election_id=election_id).exists()):
            ...
            if election_type == 'PC-GENERAL':
                ...
                for i in v:
                    voters_voted = Voted(election_id=election_id, voter_id=i.voterid_no, state=i.state,
                                         constituency=i.parliamentary,
                                         has_voted=has_voted)
                    voters_voted.save()
    ...
    if request.method == 'POST':
        ...
        if (Voted.objects.filter(election_id=election_id, voter_id=vid).exists()):
            ...
            if v.has_voted == 'no':
                ...
                return render(request, 'admin/voting.html',
                              {'username': adminhome.username, 'image': adminhome.adminimage, 'where': v.where_voted})
        else:
...
def completeelection(request):
    if Election.objects.filter(status='active'):
        ...
        return render(request, 'admin/completeelection.html',
                      {'elections': e, 'username': adminhome.username, 'image': adminhome.adminimage})
    else:
...
def modifyelection(request):
    if Election.objects.filter(status='active'):
        e = Election.objects.filter(status='active')
        return render(request, 'admin/modifyelection.html',
                      {'elections': e, 'username': adminhome.username, 'image': adminhome.adminimage})
    else:
        messages.info(request, 'No elections running')
        return render(request, 'admin/modifyelection.html',
...
def viewcandidatesforaddvote(request):
    if request.method == 'POST':
        ...
        return render(request, 'admin/addevmvotes.html',
                      {'candidates': candidates, 'username': adminhome.username, 'image': adminhome.adminimage})
...
def viewresult(request):
    ...
    return render(request, 'admin/viewresult.html',
                  {'elections': elections, 'username': adminhome.username, 'image': adminhome.adminimage})
...
def view_result(request):
    if request.method == "POST":
        ...
        if resulttype == "partywise":
            ...
            for i in won:
                total += i
            return render(request, 'admin/viewpartywise.html',
                          {'total': total, 'parwon': parwon, 'electionid': election_id, 'state': estate,
                           'username': adminhome.username, 'image': adminhome.adminimage})
            ...
            for i in v:
                if i.constituency not in constituencies:
                    constituencies.append(i.constituency)
            return render(request, 'admin/viewresultconwise.html',
                          {'electionid': election_id, 'state': estate, 'constituency': constituencies,
                           'username': adminhome.username, 'image': adminhome.adminimage})
...
def view_result_filter(request):
    if request.method == "POST":
        ...
        return render(request, 'admin/viewresultconwise.html',
                      {'totalonline': totalonline, 'totalevm': totalevm, 'totalvotes': totalvotes,
                       'result': finalresult, 'electionid': election_id, 'state': estate,
                       'constituency': constituencies, 'username': adminhome.username, 'image': adminhome.adminimage})
...
def generate_report(request):
    ...
    if Voted.objects.filter(election_id=election_id, state=state, constituency=constituency):
        ...
        if Voted.objects.filter(election_id=election_id, state=state, constituency=constituency, has_voted=has_voted):
            vvotedyes = Voted.objects.filter(election_id=election_id, state=state, constituency=constituency,
                                             has_voted=has_voted)
            v_male = 0
            v_female = 0
            v_others = 0
            ...
            if e_total != 0:
                ...
                p_total = 0
                poll_total = float("{:.2f}".format(p_total))
            report = Reports(election_id=election_id, state=state, constituency=constituency, electors_male=e_male,
                             electors_female=e_female, electors_others=e_others, electors_total=e_total,
                             voters_male=v_male, voters_female=v_female, voters_others=v_others, voters_total=v_total,
                             poll_male=poll_male, poll_female=poll_female, poll_others=poll_others,
                             poll_total=poll_total)
            report.save()
            messages.info(request, 'report generated')
            return render(request, 'admin/generatereport.html',
                          {'username': adminhome.username, 'image': adminhome.adminimage})
        else:
...
def view_report(request):
    ...
    elections = Election.objects.all()
    return render(request, 'admin/viewreport.html',
                  {'report': report, 'elections': elections, 'username': adminhome.username,
                   'image': adminhome.adminimage})
        ...
        if constituency1 == 'Parliamentary':
            ...
            if candidates:
                return render(request, 'admin/view candidate.html',
                              {'constituency': constituency2, 'candidates': candidates, 'username': adminhome.username,
                               'image': adminhome.adminimage})
            else:
...
def view_voter(request):
    if request.method == 'POST':
        ...
        if constituency1 == 'Parliamentary':
            ...
            return render(request, 'admin/view voter.html',
                          {'voters': voters, 'username': adminhome.username, 'image': adminhome.adminimage})
        else:
    ...
    if request.method == 'POST':
        if request.POST.get('editvoter'):
            ...
            if Voters.objects.filter(voterid_no=voter_id):
                ...
                return render(request, 'admin/edit voterdetails.html',
                              {'voterid_no': voter.voterid_no, 'name': voter.name,
                               ...
                               'assembly': voter.assembly, 'voter_image': voter.voter_image,
                               'username': adminhome.username, 'image': adminhome.adminimage})
            else:
                messages.info(request, 'Voter not found')
                return render(request, 'admin/edit voter.html',
                              {'username': adminhome.username, 'image': adminhome.adminimage})
    ...
    if request.method == 'POST':
        if request.POST.get('editcandidate'):
            ...
            if Candidates.objects.filter(candidate_id=candidate_id):
                ...
                if candidate.constituency == "Parliamentary":
                    return render(request, 'admin/edit candidatedetails.html',
                                  {'candidate_id': candidate.candidate_id, 'name': candidate.name,
                                   ...
                                   'party_image': candidate.party_image, 'affidavit': candidate.affidavit,
                                   'username': adminhome.username, 'image': adminhome.adminimage})
                else:
                    return render(request, 'admin/edit candidatedetails.html',
                                  {'candidate_id': candidate.candidate_id, 'name': candidate.name,
                ...
                return render(request, 'admin/edit candidate.html',
                              {'username': adminhome.username, 'image': adminhome.adminimage})
...
def viewcomplain(request):
    if Complain.objects.filter(viewed=False, replied=False):
        ...
        return render(request, 'admin/viewcomplain.html',
                      {'complain': allcomplain, 'username': adminhome.username, 'image': adminhome.adminimage})
    else:
...
def replycomplain(request):
    if Complain.objects.filter(viewed=False, replied=False):
        allcomplain = Complain.objects.filter(viewed=False, replied=False)
        return render(request, 'admin/replycomplain.html',
                      {'complain': allcomplain, 'username': adminhome.username, 'image': adminhome.adminimage})
    else:
        messages.info(request, 'No New Complains')
        return render(request, 'admin/viewcomplain.html',
...
def reply_complain(request):
    if request.method == "POST":
        ...
        allcomplain = Complain.objects.filter(viewed=False, replied=False)
        return render(request, 'admin/replycomplain.html',
                      {'complainid': complain_id, 'complain': allcomplain, 'username': adminhome.username,
                       'image': adminhome.adminimage})
...
def achangepassword(request):
    return render(request, 'admin/achangepassword.html',
                  {'username': adminhome.username, 'image': adminhome.adminimage})
